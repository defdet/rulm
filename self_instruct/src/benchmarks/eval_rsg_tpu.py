...
Используя контекст, ответь одним словом на вопрос: {question}. Ты обязан ответить "да" или "нет".'''
...
Используя текст, ответь одним словом на вопрос: Вероятно ли утверждение при условии остального текста?. Ты обязан ответить "да" или "нет".'''
...
Какое имя человека или название организации или название места должно быть вместо {mask} в запросе? Выведи только имя человека или название организации или название места и ничего больше."""
...
def main(
    ...
    if model_name not in ("gpt-4", "gpt-3.5-turbo"):
        model, params, tokenizer, generation_config = load_easydel(model_name)
